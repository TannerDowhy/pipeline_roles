...
jid4=$(sbatch -A "$2" --output="$1"/data/dada/sample_inference.out "$1"/data/dada/dada_sample_inference.sh "$1"/data/bowtie2/out "$len" "$1"/data/dada/plot.png "$1"/data/dada/seq_tab.csv "$1"/data/dada/seq_tab.rds);
